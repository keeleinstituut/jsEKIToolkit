...
EKIToolkit.prototype.modules.Quant = function(EKI, config) {
	/**
	 * This is the QuantAnalyzer factory
	 */
	"use strict";
	
	// See on moodul, lisame talle üldisi mooduli võimeid (init, destroy, notify)
	EKI.modules.addGenericModuleParts(EKI.modules.Quant);
	...
	function analyze() {
		...
		if (analyzeBatch.length > 0) {
			...
			EKI.notify({type: "quant-analysis-ALLREADY-ready"});
		}
	}
	...
	function onJSONReplySUCCESS(data, textStatus, jqXHR) {
		...
		//~ notifyObservers();
		EKI.notify({type: "quant-analysis-ready"});
	}
	...
	// Lisame mooduli liidese otse EKIToolkit objekti alla kui 'Quant'
	EKI.Quant = {
		/* this is the public interface */
		setConfig: setConfig,
		emptyTokenStorage: emptyTokenStorage,
		addToken: addToken,
		getTokenQuantList: getTokenQuantList,
		analyze: analyze,
		getPosName: posNames,
		getFormName: formNames,
		cleanToken: cleanToken,
		init:    EKI.modules.Quant.init,
		destroy: EKI.modules.Quant.destroy,
		notify:  EKI.modules.Quant.notify, // @todo: kas see on õige?
	};
